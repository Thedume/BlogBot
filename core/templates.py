def format_song_post(track_info: dict, youtube_url: str, lyrics: str, impression: str, title_kr: str = "", manual_artist: str = "") -> str:
    """J-POP 노래 소개 티스토리 HTML 템플릿을 반환합니다.
    track_info, youtube_url은 없을 수 있으므로 조건부로 섹션을 구성합니다.
    title_kr: 제목 한국어 해석 (선택). 입력 시 곡명 뒤에 (한글제목) 형태로 붙습니다.
    manual_artist: 사용자가 직접 입력한 아티스트 (선택). 입력 시 Spotify 아티스트보다 우선합니다.
    """
    album_image_url = track_info.get("album_image", "")

    # 최종적으로 사용할 아티스트: 사용자가 직접 입력했으면 그 값, 아니면 Spotify 값 그대로
    effective_artist = manual_artist.strip() if manual_artist.strip() else track_info.get("artist", "")
    ...
    if track_info.get("name"):
        name_display = track_info["name"]
        if title_kr.strip():
            name_display = f'{name_display}({title_kr.strip()})'

        info_section = (
            f'<p data-ke-size="size16">🎶&nbsp;노래&nbsp;정보 <br />'
            f'-&nbsp;곡명&nbsp;: {name_display}<br />'
            f'-&nbsp;아티스트&nbsp;: {effective_artist}<br />'
    ...
    tag_artist = f'&nbsp;#{effective_artist}' if effective_artist else ''
...
def build_song_title(jp_title: str, kr_title: str, artist: str) -> str:
    """블로그 제목 조합 로직.
    jp_title: Spotify 원제 (없을 수 있음)
    kr_title: 한국어 해석 (선택, 없을 수 있음)
    artist: 아티스트 (직접입력 우선, 없으면 Spotify 값)
    """
    jp_title = jp_title.strip() if jp_title else ""
    kr_title = kr_title.strip() if kr_title else ""
    artist = artist.strip() if artist else ""

    if jp_title and kr_title:
        name_part = f"{jp_title}({kr_title})"
    elif jp_title:
        name_part = jp_title
    elif kr_title:
        name_part = f"({kr_title})"
    else:
        name_part = ""

    if name_part and artist:
        return f"{name_part} - {artist}"
    if name_part:
        return name_part
    if artist:
        return artist
